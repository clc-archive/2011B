...
class nggSlideshowWidget extends WP_Widget {
	...
	function render_slideshow($galleryID, $irWidth = '', $irHeight = '') {
		...
		$swfobject->add_flashvars( 'file', urlencode( trailingslashit( home_url() ) . 'index.html?callback=imagerotator&gid=' . $galleryID ) );
		...
		if (is_array($imageList)){
			foreach($imageList as $image) {
				...
				if ( $instance['show'] == 'original' )
					$out .= '<img src="' . trailingslashit( home_url() ) . 'index.html?callback=image&amp;pid='.$image->pid.'&amp;width='.$instance['width'].'&amp;height='.$instance['height']. '" title="'.$alttext.'" alt="'.$alttext.'" />';
				else	
